...
export const vehicleExpenseController = {
  ...
  async getAll(_req: Request, res: Response) {
    try {
      const expenses = await vehicleExpenseService.getAll();
      res.json(expenses);
    } catch (error) {
  ...
  async update(req: Request, res: Response) {
    try {
      const updated = await vehicleExpenseService.update(req.params.id, req.body);
      if (!updated) {
        return notFound(res, 'Despesa não encontrada');
      }
      res.json(updated);
  ...
  async delete(req: Request, res: Response) {
    try {
      const deleted = await vehicleExpenseService.delete(req.params.id);
      if (!deleted) {
        return notFound(res, 'Despesa não encontrada');
      }
