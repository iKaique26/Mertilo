...
export const accountController = {
  ...
  async getById(req: Request, res: Response) {
    try {
      ...
      const account = await accountService.getById(req.params.id, userId);
      if (!account) {
  ...
  async create(req: Request, res: Response) {
    try {
      ...
      const userId = (req as any).user?.id;
      const account = await accountService.create(nome, valor, categoria, isFixed, userId);
  ...
  async update(req: Request, res: Response) {
    try {
      const userId = (req as any).user?.id;
      const updated = await accountService.update(req.params.id, req.body, userId);
      if (!updated) {
        return notFound(res, 'Conta não encontrada');
      }
      res.json(updated);
  ...
  async delete(req: Request, res: Response) {
    try {
      const userId = (req as any).user?.id;
      const deleted = await accountService.delete(req.params.id, userId);
      if (!deleted) {
        return notFound(res, 'Conta não encontrada');
      }
