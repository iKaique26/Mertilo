...
export const categoryController = {
  ...
  async getById(req: Request, res: Response) {
    try {
      ...
      const category = await categoryService.getById(req.params.id, userId);
      if (!category) {
  ...
  async create(req: Request, res: Response) {
    try {
      ...
      const userId = (req as any).user?.id;
      const category = await categoryService.create(nome, cor, percentual, userId);
  ...
  async update(req: Request, res: Response) {
    try {
      const userId = (req as any).user?.id;
      const updated = await categoryService.update(req.params.id, req.body, userId);
      if (!updated) {
        return notFound(res, 'Categoria não encontrada');
      }
      res.json(updated);
  ...
  async delete(req: Request, res: Response) {
    try {
      const userId = (req as any).user?.id;
      const deleted = await categoryService.delete(req.params.id, userId);
      if (!deleted) {
        return notFound(res, 'Categoria não encontrada');
      }
